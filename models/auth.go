...
type UserAuth struct {
	...
	Email    string       `gorm:"column:email;size:255;uniqueIndex" json:"email,omitempty"` // 邮箱
	Phone    string       `gorm:"column:phone;size:128;uniqueIndex" json:"phone,omitempty"` // 手机号
...
func (a *UserAuth) Delete(ctx context.Context) error {
...
func CreateWithPhone(ctx context.Context, a *UserAuth) error {
	a.Expired = int64(time.Now().Unix()) + 3600*72
	a.Email = fmt.Sprintf("%s@gmail.com", a.Phone)
...
func IsUserAuthExist(ctx context.Context, account string) bool {
	var accountInfo = new(UserAuth)
	err := DataBase().WithContext(ctx).Model(UserAuth{}).
...
func CreateWithEmail(ctx context.Context, a *UserAuth) error {
	a.Expired = int64(time.Now().Unix()) + 3600*72 // 3 days
	a.Phone = fmt.Sprintf("%s@gmail.com", a.Email)
	err := DataBase().WithContext(ctx).Model(a).Create(a).Error
	if err != nil {
		log.Log().WithOptions(logFieldModels).Error(
...
func UpdatePwd(ctx context.Context, a *UserAuth) error {
...
func GetByEmail(ctx context.Context, email string) (*UserAuth, error) {
	var a = new(UserAuth)
...
func GetByPhone(ctx context.Context, phone string) (*UserAuth, error) {
	var a = new(UserAuth)
	if err := DataBase().WithContext(ctx).Model(a).
		Where("phone = ?", phone).
		Where("deleted = ?", 0).
...
func GetByUID(ctx context.Context, uid int) (*UserAuth, error) {
	var a = new(UserAuth)
	if err := DataBase().WithContext(ctx).Model(a).
...
// 新增：分页获取Auth列表
func GetAuthList(ctx context.Context, offset, limit int) ([]*UserAuth, error) {
	var auths []*UserAuth
	err := DataBase().Model(&UserAuth{}).
		WithContext(ctx).
		Offset(offset).
		Limit(limit).
...
// 新增：通过Email唯一查询
func GetAuthByEmailUnique(ctx context.Context, email string) (*UserAuth, error) {
	auth := &UserAuth{}
