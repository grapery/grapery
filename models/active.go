...
func (a Active) String() string {
	activeData, _ := json.Marshal(a)
	return string(activeData)
}
...
// 按时间倒序获取用户的活动
func GetActiveByUserID(userID int64, pageSize int, offset int) ([]*Active, bool, int, error) {
	if offset == 0 {
		offset = 0
	}
	if pageSize == 0 {
	...
	var ret = new([]*Active)
	query := DataBase().Model(Active{}).
		Where("user_id = ? and deleted = 0", userID)
	var total int64
	err := query.Count(&total).Error
	if err != nil {
		log.Log().WithOptions(logFieldModels).Error(fmt.Sprintf("get user [%d] active count failed ", userID))
	...
	if total < int64(pageSize)*int64(offset) {
		return *ret, false, int(total), nil
	}
	return *ret, true, int(total), nil
}
...
func GetActiveListByTimeRange(start time.Time, end time.Time) (*[]*Active, error) {
	...
	if err := DataBase().Model(Active{}).
		Where("create_at < ? and  create_at > ? and deleted = 0", end, start).
...
// 按时间倒序获取用户的活动
func GetActiveListByActiveType(targetUserId int64, activeType uint, pageSize int, pageNum int) (*[]*Active, bool, int, error) {
	var ret = new([]*Active)
	query := DataBase().Model(Active{}).
		Where("user_id = ? and active_type = ? and deleted = 0", targetUserId, activeType)
	var total int64
	err := query.Count(&total).Error
	if err != nil {
		log.Log().WithOptions(logFieldModels).Error(fmt.Sprintf("get user [%d] active type [%d] count failed ", targetUserId, activeType))
		return nil, false, 0, err
	}
	if err := query.Order("create_at desc").
		...
		Limit(pageSize).
		Scan(ret).Error; err != nil {
		log.Log().WithOptions(logFieldModels).Error(fmt.Sprintf("get user [%d] active type [%d] failed ", targetUserId, activeType))
		return nil, false, 0, err
	}
	if total < int64(pageSize)*int64(pageNum) {
		return ret, false, int(total), nil
	}
	return ret, true, int(total), nil
}
...
// 按时间倒序获取群组的活动
func GetActiveByGroupID(ctx context.Context, groupID int64, pageSize int, pageNum int) (*[]*Active, bool, int, error) {
	var ret = new([]*Active)
	query := DataBase().Model(Active{}).WithContext(ctx).
		Where("group_id = ? and deleted = 0", groupID)
	var total int64
	err := query.Count(&total).Error
	if err != nil {
		log.Log().WithOptions(logFieldModels).Error(fmt.Sprintf("get group [%d] active count failed ", groupID))
		return nil, false, 0, err
	}
	if err := query.
		Order("create_at desc").
...
// 按时间倒序获取故事的活动
func GetActiveByStoryID(storyID int64, pageSize int, pageNum int) (*[]*Active, bool, int, error) {
	var ret = new([]*Active)
	query := DataBase().Model(Active{}).
	...
	err := query.Count(&total).Error
	if err != nil {
		log.Log().WithOptions(logFieldModels).Error(fmt.Sprintf("get story [%d] active count failed ", storyID))
		return nil, false, 0, err
	}
	if err := query.Order("create_at desc").
		Offset((pageNum - 1) * pageSize).
...
// 按时间倒序获取用户关注的小组的活动
func GetActiveByFollowingGroupID(userID int64, groupIds []int64, page, pageSize int) ([]*Active, bool, int, error) {
	...
	if total < int64(pageSize)*int64(page) {
		return ret, false, int(total), nil
	}
	return ret, true, int(total), nil
}
...
// 按时间倒序获取用户关注的故事的活动
func GetActiveByFollowingStoryID(userID int64, storyIds []int64, page, pageSize int) ([]*Active, bool, int, error) {
	var ret = new([]*Active)
	if err := DataBase().Model(Active{}).
...
func GetUserActiveHeatmapEntries(ctx context.Context, userID int64, start, end time.Time) ([]HeatmapEntry, error) {
...
func GetGroupActiveHeatmapEntries(ctx context.Context, groupID int64, start, end time.Time) ([]HeatmapEntry, error) {
	entries := make([]HeatmapEntry, 0)
	err := DataBase().Model(&Active{}).
		WithContext(ctx).
		Select("DATE(create_at) AS date, COUNT(*) AS count").
...
func CountGroupActiveMembers(ctx context.Context, groupID int64, start, end time.Time) (int64, error) {
...
// 新增：分页获取Active列表
func GetActiveList(ctx context.Context, offset, limit int) ([]*Active, bool, error) {
	var actives []*Active
	err := DataBase().Model(&Active{}).
		WithContext(ctx).
		Where("deleted = 0").
	...
	if err != nil && err != gorm.ErrRecordNotFound {
		log.Log().WithOptions(logFieldModels).Error(fmt.Sprintf("get active list failed : [%s]", err.Error()))
		return nil, false, err
	}
	return actives, true, nil
}

// 新增：通过主键唯一查询
func GetActiveByID(ctx context.Context, id int64) (*Active, bool, error) {
	active := &Active{}
	err := DataBase().Model(active).
		WithContext(ctx).
		Where("id = ?", id).
	...
	if err != nil {
		...
		log.Log().WithOptions(logFieldModels).Error(fmt.Sprintf("get active [%d] failed : [%s]", id, err.Error()))
		return nil, false, fmt.Errorf("get active [%d] failed : [%s]", id, err.Error())
	}
