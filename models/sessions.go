...
func GetUserSessionBySessionId(ctx context.Context, sessionID string) (*UserSession, error) {
	log := log.Log()
	log.Info("[GetUserSessionBySessionId] 方法入口", zap.String("sessionID", sessionID))
	var session UserSession
	err := DataBase().WithContext(ctx).Model(&UserSession{}).Where("session_id = ? AND deleted = 0", sessionID).First(&session).Error
	if err == gorm.ErrRecordNotFound {
		log.Info("[GetUserSessionBySessionId] 未找到记录", zap.String("sessionID", sessionID))
		return nil, nil
	}
	if err != nil {
		log.Error("[GetUserSessionBySessionId] 查询失败", zap.Error(err))
		return nil, fmt.Errorf("get user_session by session_id failed: %w", err)
	}
	log.Info("[GetUserSessionBySessionId] 查询成功", zap.Any("session", session))
	return &session, nil
}
...
func ListUserSessionsByUserIdUsePage(ctx context.Context, userId int64, page int, pageSize int) ([]*UserSession, error) {
	log := log.Log()
	log.Info("[ListUserSessionsByUserId] 方法入口", zap.Int64("userId", userId))
	var sessions []*UserSession
	err := DataBase().WithContext(ctx).
