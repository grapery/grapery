...
func GetLastMessageBySessionID(ctx context.Context, sessionID string) (*LLMChatMsg, error) {
	var msg LLMChatMsg
	err := DataBase().
		WithContext(ctx).Model(&LLMChatMsg{}).
		Where("session_id = ? AND deleted = 0", sessionID).
		Order("created_at desc").First(&msg).Error
	if err != nil {
		return nil, fmt.Errorf("get last message by session_id failed: %w", err)
	}
	return &msg, nil
}
