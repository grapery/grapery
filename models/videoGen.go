...
type VideoGen struct {
	...
	Provider  string             `gorm:"column:provider" json:"provider,omitempty"`     // 视频生成提供商
...
func GetVideoGen(ctx context.Context, id int64) (*VideoGen, error) {
	...
	err := DataBase().Table(videoGen.TableName()).
		WithContext(ctx).
		Where("id = ?", id).First(&videoGen).Error
	if err != nil {
		return nil, err
	}
	return &videoGen, nil
...
func UpdateVideoGen(ctx context.Context, videoGen *VideoGen) error {
	return DataBase().Table(videoGen.TableName()).
		WithContext(ctx).
		Where("id = ?", videoGen.ID).Updates(videoGen).Error
}

func UpdateVideoGenFields(ctx context.Context, id int64, values map[string]interface{}) error {
	if len(values) == 0 {
		return nil
	}
	return DataBase().Table(VideoGen{}.TableName()).
...
func GetVideoGenListByStatus(ctx context.Context, status int) ([]*VideoGen, error) {
	...
	err := DataBase().Table(VideoGen{}.TableName()).
		WithContext(ctx).
		Where("gen_status = ?", status).Find(&videoGenList).Error
	if err != nil {
		return nil, err
	}
	return videoGenList, nil
