...
func Init(uname, pwd, address, dbname string) error {
	...
	database.AutoMigrate(&UserFollow{})
	...
	database.AutoMigrate(&RoleAgent{})
	database.AutoMigrate(&SystemNotification{})

	database.AutoMigrate(&ImageGen{})
	database.AutoMigrate(&VideoGen{})

	// 新增的认证相关表
	database.AutoMigrate(&PasswordReset{})
	database.AutoMigrate(&EmailVerification{})

	database.AutoMigrate(&UserAuth{})
	database.AutoMigrate(&ThirdPartyLogin{})

	database.AutoMigrate(&RolePoster{})
	database.AutoMigrate(&RolePosterLiked{})
	database.AutoMigrate(&RolePosterShared{})
	database.AutoMigrate(&RoleVideo{})
	database.AutoMigrate(&RoleVideoLiked{})
	database.AutoMigrate(&RoleVideoShared{})

	// 用户反馈相关表
	database.AutoMigrate(&UserFeedback{})
	return nil
}
