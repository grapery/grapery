-- Migration: Rename creator_id to user_id in fragments table
-- Version: 014
-- Date: 2026-02-11
...
SET @index_exists = (
    ...
    AND INDEX_NAME = 'idx_fragment_creator'
);

SET @drop_index_sql = IF(@index_exists > 0,
    'DROP INDEX idx_fragment_creator ON fragments',
    'SELECT ''Index idx_fragment_creator does not exist, skipping'' AS message'
);
...
SET @fk_exists = (
    ...
    WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'fragments'
    AND CONSTRAINT_NAME = 'fk_fragment_creator'
    AND REFERENCED_TABLE_NAME = 'users'
);

SET @drop_fk_sql = IF(@fk_exists > 0,
    'ALTER TABLE fragments DROP FOREIGN KEY fk_fragment_creator',
    'SELECT ''Foreign key fk_fragment_creator does not exist, skipping'' AS message'
);
