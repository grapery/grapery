...
在已配置 `.env` 中 MySQL 连接（`DB_*`）且迁移已就绪的前提下，可写入一批开发用数据（约 15 个用户、关注关系、故事、三层故事板链、碎片及评论）：

```bash
go run ./cmd/seed
```

- 登录账号形如 `seed_user_01` … `seed_user_15`，默认密码：`test123456`。
- 若已存在 `seed_user_%` 用户，命令会拒绝执行；可加 **`-force`** 先删除这些用户及其关联数据再重新写入：

```bash
go run ./cmd/seed -force
```

**注意**：`-force` 会硬删除种子用户相关的故事、故事板、碎片、评论等，请勿在生产库使用。
