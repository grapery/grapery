...
func (c *HuoShanCozeClient) ConversationCreate(ctx context.Context, params CozeConversationCreateParams) (string, error) {
	...
	graperylog.Log().Info("[ConversationCreate] 创建HTTP请求", zap.String("url", Endpoint+"/v1/conversation/create"), zap.String("jsonData", string(jsonData)))
	// 发送请求
	client := &http.Client{}
	...
	// 判断返回状态
	if respObj.Code != 0 {
		respObjData, _ := json.Marshal(respObj)
		graperylog.Log().Error("[ConversationCreate] 创建会话失败", zap.String("msg", respObj.Msg), zap.String("data", string(respObjData)))
