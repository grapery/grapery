...
// CalculateImageDimensions 根据图片比例计算宽度和高度
// 参数:
//   - ratio: 图片比例枚举值
//   - baseSize: 基准尺寸，默认为1000
//
// 返回:
//   - width: 计算后的宽度
//   - height: 计算后的高度
func CalculateImageDimensions(ratio api.ImageRatios, baseSize int) (width, height int) {
	if baseSize <= 0 {
		baseSize = 1000 // 默认基准尺寸
	}

	switch ratio {
	case api.ImageRatios_Ratio1_1: // 1:1 正方形
		width, height = baseSize, baseSize
	case api.ImageRatios_Ratio4_3: // 4:3 横屏
		width, height = int(float64(baseSize)*4.0/3.0), baseSize
	case api.ImageRatios_Ratio16_9: // 16:9 宽屏
		width, height = int(float64(baseSize)*16.0/9.0), baseSize
	case api.ImageRatios_Ratio3_2: // 3:2 横屏
		width, height = int(float64(baseSize)*3.0/2.0), baseSize
	case api.ImageRatios_Ratio3_4: // 3:4 竖屏
		width, height = baseSize, int(float64(baseSize)*4.0/3.0)
	case api.ImageRatios_Ratio2_3: // 2:3 竖屏
		width, height = baseSize, int(float64(baseSize)*3.0/2.0)
	default:
		// 默认使用1:1比例
		width, height = baseSize, baseSize
	}
...
func (c *HuoShanCozeClient) WorkflowRun(ctx context.Context, workflowID string, params CozeWorkflowRunParams) (interface{}, error) {
	// 序列化为JSON
	jsonData, err := json.Marshal(params)
	if err != nil {
		graperylog.Log().Error("[WorkflowRun] 参数序列化失败", zap.Error(err), zap.Any("params", params))
		return "", err
	}
	graperylog.Log().Info("[WorkflowRun] 参数序列化成功", zap.String("params", string(jsonData)))

	// 创建HTTP请求
	req, err := http.NewRequestWithContext(ctx, "POST", Endpoint+"/v1/workflow/run", bytes.NewBuffer(jsonData))
	if err != nil {
		graperylog.Log().Error("[WorkflowRun] 创建HTTP请求失败", zap.Error(err))
		return "", err
	}
	graperylog.Log().Info("[WorkflowRun] 创建HTTP请求成功", zap.String("url", Endpoint+"/v1/workflow/run"))

	// 设置请求头
	req.Header.Set("Authorization", "Bearer "+APIKey)
	req.Header.Set("Content-Type", "application/json")

	// 设置HTTP客户端，超时时间为300秒（5分钟）
	client := &http.Client{
		Timeout: 600 * time.Second, // 超时时间300秒
	}
	graperylog.Log().Info("[WorkflowRun] 发送HTTP请求")
	resp, err := client.Do(req)
	if err != nil {
		graperylog.Log().Error("[WorkflowRun] HTTP请求发送失败", zap.Error(err))
		return "", err
	}
	defer resp.Body.Close()
	graperylog.Log().Info("[WorkflowRun] HTTP请求发送成功", zap.Int("status", resp.StatusCode))

	// 读取响应体
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		graperylog.Log().Error("[WorkflowRun] 读取响应体失败", zap.Error(err))
		return "", err
	}
	graperylog.Log().Info("[WorkflowRun] 读取响应体成功", zap.ByteString("body", body))

	// 解析响应
	var apiResp CozeWorkflowRunResponse
	err = json.Unmarshal(body, &apiResp)
	if err != nil {
		graperylog.Log().Error("[WorkflowRun] 响应体解析失败", zap.Error(err), zap.ByteString("body", body))
		return "", err
	}
	graperylog.Log().Info("[WorkflowRun] 响应体解析成功", zap.Any("apiResp", apiResp))

	// 判断API返回状态
	if apiResp.Code != 0 {
		graperylog.Log().Error("[WorkflowRun] API返回错误", zap.Int("code", apiResp.Code), zap.String("msg", apiResp.Msg))
		return "", errors.New(apiResp.Msg)
	}
	graperylog.Log().Info("[WorkflowRun] API返回成功", zap.String("data", apiResp.Data))

	ret, err := ParseCozeOutput(apiResp.Data)
	if err != nil {
		graperylog.Log().Error("[WorkflowRun] 解析Coze输出失败", zap.Error(err), zap.String("data", apiResp.Data))
		return "", err
	}
	graperylog.Log().Info("[WorkflowRun] 解析Coze输出成功", zap.Any("ret", ret))
...
type CozeStoryRolePosterImageParams struct {
	RoleName  string
	RoleDesc  string
	RoleImage string
	Style     string
...
func (c *HuoShanCozeClient) StoryRolePosterImage(ctx context.Context, params CozeStoryRolePosterImageParams) (string, error) {
	workflowID := "7538220459730632704"
	apiParams := CozeWorkflowRunParams{
		WorkflowID: workflowID,
		Parameters: map[string]interface{}{
			"app_id":     APPID,
	...
	apiParamsData, _ := json.Marshal(apiParams)
	log.Log().Info("[StoryRolePosterImage] 调用工作流参数", zap.String("workflowID", workflowID), zap.Any("apiParams", string(apiParamsData)))
	ret, err := c.WorkflowRun(ctx, workflowID, apiParams)
	if err != nil {
		return "", err
	}
...
func (c *HuoShanCozeClient) StoryRoleImage(ctx context.Context, params CozeStoryRoleImageParams) (string, error) {
	...
	// 根据Ratio参数计算Width和Height，以1000为基数
	width, height := CalculateImageDimensions(params.Ratio, 1000)
	...
	ret, err := c.WorkflowRun(ctx, workflowID, apiParams)
	if err != nil {
		graperylog.Log().Error("[StoryRoleImage] 工作流执行失败", zap.Error(err))
		return "", err
	}
	graperylog.Log().Info("[StoryRoleImage] 工作流执行成功", zap.String("workflowID", workflowID), zap.Any("result", ret))
	strRet := ret.([]interface{})
	return strRet[0].(string), nil
}
...
func (c *HuoShanCozeClient) StoryboardVideo(ctx context.Context, params CozeStoryboardVideoParams) (string, error) {
	...
	apiParams := CozeWorkflowRunParams{
		...
		Parameters: map[string]interface{}{
			"app_id":          APPID,
			"prompt":          params.Prompt,
			"start_ref_image": params.StartRefImage,
			"end_ref_image":   params.EndRefImage,
			"style":           params.Style,
		},
	}
	ret, err := c.WorkflowRun(ctx, workflowID, apiParams)
	if err != nil {
		log.Log().Info("[StoryboardVideo] 工作流执行失败", zap.Error(err))
		return "", err
	}
	log.Log().Info("[StoryboardVideo] 工作流执行成功", zap.String("workflowID", workflowID), zap.Any("result", ret))
	return ret.(string), nil
}
...
func (c *HuoShanCozeClient) StoryboardWriter(ctx context.Context, params CozeStoryboardWriterParams) (string, error) {
	...
	apiParams := CozeWorkflowRunParams{
		...
		Parameters: map[string]interface{}{
			...
			"prev_content":     params.PrevContent,
...
func (c CozeStoryboardContinueParams) RoleStr() string {
	data, _ := json.Marshal(c.Roles)
	return string(data)
}
...
func ParseCozeOutput(output string) (map[string]interface{}, error) {
	// 1. 去掉前后的 --- 分隔符和多余空白
...
func (c *HuoShanCozeClient) ChatWithRoleStream(ctx context.Context, params CozeChatWithRoleStreamParams, msgChan chan string, answerMap map[string][]AnswerOrFollowUp) error {
	...
	for {
		...
		if err != nil {
			graperylog.Log().Error("[ChatWithRoleStream] 读取SSE流式响应失败", zap.Error(err), zap.String("line", line))
			return err
		}
...
func (c *HuoShanCozeClient) GenerateStoryboardVideo(ctx context.Context, params CozeStoryboardVideoParams) (string, error) {
