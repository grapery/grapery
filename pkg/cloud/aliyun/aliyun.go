...
var (
	...
	// Preferred: ALIYUN_ACCESS_KEY_ID / ALIYUN_ACCESS_KEY_SECRET (see env.example).
	// Fallbacks kept for older local configs: ALIYUN_API_KEY / ALIYUN_SECRET_KEY.
	APIKey       = firstEnv("ALIYUN_ACCESS_KEY_ID", "ALIYUN_API_KEY")
	SecretKey    = firstEnv("ALIYUN_ACCESS_KEY_SECRET", "ALIYUN_SECRET_KEY")
	Endpoint     = firstEnv("ALIYUN_OSS_ENDPOINT", "ALIYUN_ENDPOINT")
	Bucket       = firstEnv("ALIYUN_OSS_BUCKET", "ALIYUN_BUCKET")
...
func NewAliyunClient() (*AliyunClient, error) {
	if APIKey == "" || SecretKey == "" || Bucket == "" {
		return nil, errors.New("aliyun credentials not set: require ALIYUN_ACCESS_KEY_ID/ALIYUN_ACCESS_KEY_SECRET (or ALIYUN_API_KEY/ALIYUN_SECRET_KEY) and ALIYUN_OSS_BUCKET")
	}
...
func (c *AliyunClient) UploadVideoBytes(objectKey string, data []byte) (string, error) {
	if len(data) == 0 {
...
func (c *AliyunClient) PersistImages(url string) error {
	imagePath := strings.Split(url, "/")
	imageId := imagePath[len(imagePath)-1]
	levelDetail, err := c.PersistMultiLevelImages(imageId)
	if err != nil {
		log.Log().Error("[PersistImages] 持久化图片失败", zap.Error(err), zap.String("imageId", imageId))
		return err
	}
	levelData, _ := json.Marshal(levelDetail)
	log.Log().Info("[PersistImages] 持久化图片成功", zap.String("imageId", imageId), zap.Any("levelData", string(levelData)))
	return nil
}
...
func (c *AliyunClient) PersistMultiLevelImages(objectKey string) (ImageLevels, error) {
	...
	log.Log().Info("[PersistMultiLevelImages] 持久化多级图片",
