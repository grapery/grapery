...
type TokenUsageService interface {
	// 记录Token使用
	RecordTokenUsage(ctx context.Context, userID int64, usageType vippay.TokenUsageType, inputTokens, outputTokens int64, modelName, featureName string) error

	// 记录失败请求
	RecordFailedRequest(ctx context.Context, userID int64, usageType vippay.TokenUsageType) error

	// 检查用户用量限制
	CheckUserUsageLimit(ctx context.Context, userID int64, usageType vippay.TokenUsageType) (*UsageLimitResult, error)

	// 获取用户用量统计
	GetUserUsageStats(ctx context.Context, userID int64, period vippay.TokenUsagePeriod) (map[string]interface{}, error)

	// 获取用户各类型用量
	GetUserUsageByType(ctx context.Context, userID int64, period vippay.TokenUsagePeriod) (map[vippay.TokenUsageType]map[string]interface{}, error)

	// 获取用户当前周期用量
	GetUserCurrentUsage(ctx context.Context, userID int64, usageType vippay.TokenUsageType, period vippay.TokenUsagePeriod) (*vippay.TokenUsage, error)

	// 检查是否可以执行操作
	CanPerformAction(ctx context.Context, userID int64, usageType vippay.TokenUsageType) (*ActionPermission, error)
}
...
func (s *TokenUsageServiceImpl) RecordTokenUsage(ctx context.Context, userID int64, usageType vippay.TokenUsageType, inputTokens, outputTokens int64, modelName, featureName string) error {
	...
	// 记录日用量
	err := vippay.IncrementTokenUsage(ctx, userID, usageType, vippay.TokenUsagePeriodDaily, inputTokens, outputTokens, modelName, featureName)
	if err != nil {
	...
	// 记录月用量
	err = vippay.IncrementTokenUsage(ctx, userID, usageType, vippay.TokenUsagePeriodMonthly, inputTokens, outputTokens, modelName, featureName)
	if err != nil {
	...
	// 记录年用量
	err = vippay.IncrementTokenUsage(ctx, userID, usageType, vippay.TokenUsagePeriodYearly, inputTokens, outputTokens, modelName, featureName)
	if err != nil {
...
func (s *TokenUsageServiceImpl) RecordFailedRequest(ctx context.Context, userID int64, usageType vippay.TokenUsageType) error {
	s.logger.WithFields(logrus.Fields{
		"user_id":    userID,
		"usage_type": usageType,
	...
	// 记录日失败请求
	err := vippay.IncrementFailedRequest(ctx, userID, usageType, vippay.TokenUsagePeriodDaily)
	if err != nil {
	...
	// 记录月失败请求
	err = vippay.IncrementFailedRequest(ctx, userID, usageType, vippay.TokenUsagePeriodMonthly)
	if err != nil {
	...
	// 记录年失败请求
	err = vippay.IncrementFailedRequest(ctx, userID, usageType, vippay.TokenUsagePeriodYearly)
	if err != nil {
...
func (s *TokenUsageServiceImpl) CheckUserUsageLimit(ctx context.Context, userID int64, usageType vippay.TokenUsageType) (*UsageLimitResult, error) {
	// 获取用户当前活跃订阅
	...
	// 获取用户当前月用量
	currentUsage, err := s.GetUserCurrentUsage(ctx, userID, usageType, vippay.TokenUsagePeriodMonthly)
	if err != nil {
...
func (s *TokenUsageServiceImpl) GetUserUsageStats(ctx context.Context, userID int64, period vippay.TokenUsagePeriod) (map[string]interface{}, error) {
	return vippay.GetUserTokenUsageStats(ctx, userID, period)
}

// GetUserUsageByType 获取用户各类型用量
func (s *TokenUsageServiceImpl) GetUserUsageByType(ctx context.Context, userID int64, period vippay.TokenUsagePeriod) (map[vippay.TokenUsageType]map[string]interface{}, error) {
	return vippay.GetUserTokenUsageByType(ctx, userID, period)
}

// GetUserCurrentUsage 获取用户当前周期用量
func (s *TokenUsageServiceImpl) GetUserCurrentUsage(ctx context.Context, userID int64, usageType vippay.TokenUsageType, period vippay.TokenUsagePeriod) (*vippay.TokenUsage, error) {
	usage, err := vippay.GetUserCurrentPeriodUsage(ctx, userID, usageType, period)
	if err != nil {
		// 如果没有记录，返回空用量
		return &vippay.TokenUsage{
			UserID:       userID,
			UsageType:    usageType,
			Period:       period,
			InputTokens:  0,
...
func (s *TokenUsageServiceImpl) CanPerformAction(ctx context.Context, userID int64, usageType vippay.TokenUsageType) (*ActionPermission, error) {
	// 检查用量限制
...
func (s *TokenUsageServiceImpl) getUsageLimitBySubscriptionAndType(subscription *vippay.UserSubscription, usageType vippay.TokenUsageType) int64 {
	// 基础限制来自订阅的QuotaLimit
...
func (s *TokenUsageServiceImpl) getDefaultUsageLimit(usageType vippay.TokenUsageType) *UsageLimitResult {
...
func (s *TokenUsageServiceImpl) getUpgradeSuggestion(usageType vippay.TokenUsageType) string {
	switch usageType {
	case vippay.TokenUsageTypeChat:
