...
type IAPProductService interface {
	// 产品管理
	CreateProduct(ctx context.Context, product *models.IAPProduct) error
	GetProduct(ctx context.Context, id uint) (*models.IAPProduct, error)
	GetProductByProductID(ctx context.Context, productID string) (*models.IAPProduct, error)
	GetProductBySKU(ctx context.Context, sku string, platform models.IAPProductPlatform) (*models.IAPProduct, error)
	UpdateProduct(ctx context.Context, product *models.IAPProduct) error
	DeleteProduct(ctx context.Context, id uint) error

	// 产品查询
	GetActiveProducts(ctx context.Context, platform models.IAPProductPlatform) ([]*models.IAPProduct, error)
	GetProductsByType(ctx context.Context, productType models.IAPProductType, platform models.IAPProductPlatform) ([]*models.IAPProduct, error)
	GetFeaturedProducts(ctx context.Context, platform models.IAPProductPlatform) ([]*models.IAPProduct, error)
	GetProductStats(ctx context.Context, platform models.IAPProductPlatform) (map[string]interface{}, error)
...
// CreateProduct 创建产品
func (s *IAPProductServiceImpl) CreateProduct(ctx context.Context, product *models.IAPProduct) error {
	...
	return models.CreateIAPProduct(ctx, product)
}

// GetProduct 获取产品
func (s *IAPProductServiceImpl) GetProduct(ctx context.Context, id uint) (*models.IAPProduct, error) {
	return models.GetIAPProduct(ctx, id)
}

// GetProductByProductID 根据产品ID获取产品
func (s *IAPProductServiceImpl) GetProductByProductID(ctx context.Context, productID string) (*models.IAPProduct, error) {
	return models.GetIAPProductByProductID(ctx, productID)
}

// GetProductBySKU 根据SKU获取产品
func (s *IAPProductServiceImpl) GetProductBySKU(ctx context.Context, sku string, platform models.IAPProductPlatform) (*models.IAPProduct, error) {
	return models.GetIAPProductBySKU(ctx, sku, platform)
}

// UpdateProduct 更新产品
func (s *IAPProductServiceImpl) UpdateProduct(ctx context.Context, product *models.IAPProduct) error {
	s.logger.WithFields(logrus.Fields{
		"product_id": product.ProductID,
		"platform":   product.Platform,
		"name":       product.Name,
	...
	return models.UpdateIAPProduct(ctx, product)
}
...
func (s *IAPProductServiceImpl) DeleteProduct(ctx context.Context, id uint) error {
	...
	return models.DeleteIAPProduct(ctx, id)
}

// GetActiveProducts 获取激活的产品
func (s *IAPProductServiceImpl) GetActiveProducts(ctx context.Context, platform models.IAPProductPlatform) ([]*models.IAPProduct, error) {
	return models.GetActiveIAPProducts(ctx, platform)
}

// GetProductsByType 根据类型获取产品
func (s *IAPProductServiceImpl) GetProductsByType(ctx context.Context, productType models.IAPProductType, platform models.IAPProductPlatform) ([]*models.IAPProduct, error) {
	return models.GetIAPProductsByType(ctx, productType, platform)
}

// GetFeaturedProducts 获取推荐产品
func (s *IAPProductServiceImpl) GetFeaturedProducts(ctx context.Context, platform models.IAPProductPlatform) ([]*models.IAPProduct, error) {
	return models.GetFeaturedIAPProducts(ctx, platform)
}

// GetProductStats 获取产品统计
func (s *IAPProductServiceImpl) GetProductStats(ctx context.Context, platform models.IAPProductPlatform) (map[string]interface{}, error) {
	return models.GetIAPProductStats(ctx, platform)
}
...
func (s *IAPProductServiceImpl) SyncProductsFromApple(ctx context.Context) error {
	...
	// 1. 获取现有的Apple产品列表
	existingProducts, err := models.GetActiveIAPProducts(ctx, models.IAPProductPlatformApple)
	if err != nil {
...
func (s *IAPProductServiceImpl) SyncProductsFromGoogle(ctx context.Context) error {
	...
	// 1. 获取现有的Google产品列表
	existingProducts, err := models.GetActiveIAPProducts(ctx, models.IAPProductPlatformGoogle)
	if err != nil {
...
func (s *IAPProductServiceImpl) UpdateSyncStatus(ctx context.Context, id uint, syncStatus, syncError string) error {
	return models.UpdateIAPProductSyncStatus(ctx, id, syncStatus, syncError)
}
...
func (s *IAPProductServiceImpl) CreateDefaultProducts(ctx context.Context) error {
	...
	defaultProducts := []*models.IAPProduct{
		{
			ProductID:    "com.rankquantity.voyager.sub_basic_y",
			Platform:     models.IAPProductPlatformApple,
			ProductType:  models.IAPProductTypeSubscription,
		...
		{
			ProductID:    "com.rankquantity.voyager.sub_basic_y",
			Platform:     models.IAPProductPlatformGoogle,
			ProductType:  models.IAPProductTypeSubscription,
			Name:         "Basic Annual Subscription",
			Description:  "Basic annual subscription with essential features",
			Price:        99.99,
	...
	for _, product := range defaultProducts {
		// 检查产品是否已存在
		_, err := models.GetIAPProductByProductID(ctx, product.ProductID)
		if err == nil {
...
// parseApplePrivateKey 解析Apple私钥
func (s *IAPProductServiceImpl) parseApplePrivateKey(privateKeyPEM string) (*rsa.PrivateKey, error) {
	// 解码PEM格式的私钥
...
// createAppleJWT 创建Apple JWT token
func (s *IAPProductServiceImpl) createAppleJWT(claims *AppleJWTClaims, privateKey *rsa.PrivateKey) (string, error) {
	// TODO: 实现JWT创建和签名
	// 这里需要实现JWT的创建、编码和签名
	// 可以使用 golang-jwt/jwt 库或手动实现

	// 目前返回模拟token
	s.logger.Warn("JWT创建功能需要实现，返回模拟token")
	return "mock_apple_jwt_token", nil
}
...
func (s *IAPProductServiceImpl) syncAppleProducts(ctx context.Context, existingProducts []*models.IAPProduct, appleProducts []*AppleProductInfo) (int, int, int, error) {
	s.logger.Info("Syncing Apple products to database")

	var syncedCount, updatedCount, createdCount int

	// 创建现有产品的映射，便于查找
	existingMap := make(map[string]*models.IAPProduct)
...
func (s *IAPProductServiceImpl) syncGoogleProducts(ctx context.Context, existingProducts []*models.IAPProduct, googleProducts []*GoogleProductInfo) (int, int, int, error) {
...
func (s *IAPProductServiceImpl) updateAppleProduct(ctx context.Context, existingProduct *models.IAPProduct, appleProduct *AppleProductInfo) error {
	...
	if needsUpdate {
		...
		return models.UpdateIAPProduct(ctx, existingProduct)
	}
...
func (s *IAPProductServiceImpl) updateGoogleProduct(ctx context.Context, existingProduct *models.IAPProduct, googleProduct *GoogleProductInfo) error {
...
// 辅助函数：解析Apple产品类型
func (s *IAPProductServiceImpl) parseAppleProductType(productType string) models.IAPProductType {
	switch strings.ToUpper(productType) {
	case "SUBSCRIPTION":
		return models.IAPProductTypeSubscription
	case "NON_CONSUMABLE":
		return models.IAPProductTypeOneTime
	case "CONSUMABLE":
		return models.IAPProductTypeConsumable
	default:
		return models.IAPProductTypeOneTime
	}
}
...
// 辅助函数：解析Google产品类型
func (s *IAPProductServiceImpl) parseGoogleProductType(productType string) models.IAPProductType {
...
// 辅助函数：解析Apple产品状态
func (s *IAPProductServiceImpl) parseAppleProductStatus(state string) models.IAPProductStatus {
	switch strings.ToUpper(state) {
	case "READY_FOR_SALE":
		return models.IAPProductStatusActive
	case "MISSING_METADATA", "PREPARE_FOR_SUBMISSION":
		return models.IAPProductStatusInactive
	default:
		return models.IAPProductStatusInactive
	}
}
...
// 辅助函数：解析Google产品状态
func (s *IAPProductServiceImpl) parseGoogleProductStatus(status string) models.IAPProductStatus {
