...
func (s *IAPPersistenceService) PersistReceiptValidation(ctx context.Context, receipt *IAPReceipt) error {
	...
	// 开始数据库事务
	tx := models.DataBase().WithContext(ctx).Begin()
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
		}
...
func (s *IAPPersistenceService) saveReceiptValidation(tx *gorm.DB, receipt *IAPReceipt) error {
	validation := &models.IAPReceiptValidation{
...
func (s *IAPPersistenceService) saveGooglePurchase(tx *gorm.DB, receipt *IAPReceipt) error {
	googlePurchase := &models.GooglePurchase{
		UserID:           receipt.UserID,
...
func (s *IAPPersistenceService) saveSubscription(tx *gorm.DB, receipt *IAPReceipt) error {
	...
	case IAPPlatformGoogle:
		var googleSub models.GoogleSubscription
...
func (s *IAPPersistenceService) updateExistingSubscription(tx *gorm.DB, receipt *IAPReceipt, existingSubscription interface{}) error {
	...
	switch receipt.Platform {
	case IAPPlatformApple:
		appleSub := existingSubscription.(*models.AppleSubscription)
	...
	case IAPPlatformGoogle:
		googleSub := existingSubscription.(*models.GoogleSubscription)
...
func (s *IAPPersistenceService) createNewSubscription(tx *gorm.DB, receipt *IAPReceipt) error {
	...
	case IAPPlatformGoogle:
		googleSub := &models.GoogleSubscription{
			UserID:        receipt.UserID,
...
func (s *IAPPersistenceService) PersistPaymentRecord(ctx context.Context, receipt *IAPReceipt, productID string, amount int64) error {
	...
	case IAPPlatformGoogle:
		paymentMethod = models.PaymentMethodGooglePay
	...
	return models.CreatePaymentRecord(ctx, paymentRecord)
}
...
func (s *IAPPersistenceService) PersistUserSubscription(ctx context.Context, receipt *IAPReceipt, productID string, packagePlanID uint) error {
	...
	return models.CreateUserSubscription(ctx, userSubscription)
}

// GetReceiptValidationHistory 获取收据验证历史
func (s *IAPPersistenceService) GetReceiptValidationHistory(ctx context.Context, userID uint64, limit int) ([]*models.IAPReceiptValidation, error) {
	var validations []*models.IAPReceiptValidation
...
func (s *IAPPersistenceService) GetUserPaymentHistory(ctx context.Context, userID uint64, limit int) ([]*models.PaymentRecord, error) {
	return models.GetUserPaymentRecords(ctx, int64(userID), 0, limit)
}

// GetUserSubscriptionHistory 获取用户订阅历史
func (s *IAPPersistenceService) GetUserSubscriptionHistory(ctx context.Context, userID uint64, limit int) ([]*models.UserSubscription, error) {
	return models.GetUserSubscriptionsByUserID(ctx, int64(userID), 0, limit)
}
