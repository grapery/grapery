...
func (m *UsageMiddleware) CheckUsageLimit(usageType vippay.TokenUsageType) gin.HandlerFunc {
...
func (m *UsageMiddleware) RecordUsage(usageType vippay.TokenUsageType) gin.HandlerFunc {
	return func(c *gin.Context) {
		// 执行后续处理
		c.Next()
...
func (h *UsageLimitHandler) GetUsageStats(c *gin.Context) {
	...
	var period vippay.TokenUsagePeriod
	switch periodStr {
	case "daily":
		period = vippay.TokenUsagePeriodDaily
	case "weekly":
		period = vippay.TokenUsagePeriodWeekly
	case "monthly":
		period = vippay.TokenUsagePeriodMonthly
	case "yearly":
		period = vippay.TokenUsagePeriodYearly
	default:
		period = vippay.TokenUsagePeriodMonthly
	}
...
func getUsageTypeString(usageType vippay.TokenUsageType) string {
	switch usageType {
...
func getUsageTypeFromString(usageTypeStr string) vippay.TokenUsageType {
	switch usageTypeStr {
	case "chat":
