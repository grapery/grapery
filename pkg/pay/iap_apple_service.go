...
func (s *AppleIAPService) GetSubscription(ctx context.Context, subscriptionID string) (*IAPSubscription, error) {
	...
	// 从数据库获取Apple订阅信息
	var appleSub models.AppleSubscription
...
func (s *AppleIAPService) SyncSubscription(ctx context.Context, subscriptionID string) error {
	...
	err = models.DataBase().WithContext(ctx).Save(appleSub).Error
	if err != nil {
...
func (s *AppleIAPService) AcknowledgePurchase(ctx context.Context, purchaseToken string) error {
	...
	// 2. 查找对应的收据记录
	var appleReceipt models.AppleReceipt
	err = models.DataBase().WithContext(ctx).
	...
	if err := models.DataBase().WithContext(ctx).Save(&appleReceipt).Error; err != nil {
	...
	if appleReceipt.ExpirationDate != nil {
		...
		if err == nil {
			...
			models.DataBase().WithContext(ctx).Save(&appleSub)
		}
	}
	...
	paymentRecord := &models.PaymentRecord{
		...
		PaymentMethod:   models.PaymentMethodApplePay,
		PaymentProvider: "App Store",
	}

	if err := models.DataBase().WithContext(ctx).Create(paymentRecord).Error; err != nil {
...
func (s *AppleIAPService) ConsumePurchase(ctx context.Context, purchaseToken string) error {
	...
	// 7. 记录消费历史
	consumptionRecord := &models.PaymentRecord{
		UserID:          int64(userID),
	...
	if err := models.DataBase().WithContext(ctx).Create(consumptionRecord).Error; err != nil {
...
func (s *AppleIAPService) handleInitialPurchase(ctx context.Context, notification *IAPNotification, transactionInfo *AppleTransactionInfo) error {
	...
	// 2. 检查订阅是否已存在
	var existingSub models.AppleSubscription
	err := models.DataBase().WithContext(ctx).
	...
	if err != nil {
		...
		// 订阅已存在，更新记录
		if err := s.updateAppleSubscriptionFromTransaction(ctx, transactionInfo, notification, func(appleSub *models.AppleSubscription) {
...
func (s *AppleIAPService) GetPurchaseHistory(ctx context.Context, userID uint64, limit int) ([]models.PaymentRecord, error) {
	s.logger.WithFields(logrus.Fields{
		"user_id": userID,
		"limit":   limit,
	}).Debug("获取Apple购买历史")
...
func (s *AppleIAPService) GetActiveSubscriptions(ctx context.Context, userID uint64) ([]models.AppleSubscription, error) {
	s.logger.WithField("user_id", userID).Debug("获取Apple活跃订阅")

	var subscriptions []models.AppleSubscription
	err := models.DataBase().WithContext(ctx).
...
func (s *AppleIAPService) TrackPurchaseEvent(ctx context.Context, eventType string, purchaseToken string, metadata map[string]interface{}) error {
	...
	// 创建事件记录
	eventRecord := &models.PaymentRecord{
		UserID:          int64(userID),
	...
	if err := models.DataBase().WithContext(ctx).Create(eventRecord).Error; err != nil {
...
func (s *AppleIAPService) GetPurchaseAnalytics(ctx context.Context, userID uint64, startDate, endDate time.Time) (map[string]interface{}, error) {
	...
	err = models.DataBase().WithContext(ctx).
		Model(&models.AppleSubscription{}).
		Where("user_id = ? AND status = ? AND (expires_date IS NULL OR expires_date > ?)",
			userID, "Active", time.Now()).
...
func (s *AppleIAPService) CleanupExpiredPurchases(ctx context.Context, olderThan time.Time) error {
	...
	result = models.DataBase().WithContext(ctx).
		...
		Delete(&models.AppleSubscription{})
	...
	result = models.DataBase().WithContext(ctx).
		...
		Delete(&models.AppleNotification{})
...
func (s *AppleIAPService) saveAppleNotification(ctx context.Context, notification *IAPNotification, status string) error {
	appleNotification := &models.AppleNotification{
	...
	return models.DataBase().WithContext(ctx).Create(appleNotification).Error
}

// updateAppleSubscriptionFromTransaction 从交易信息更新Apple订阅记录
func (s *AppleIAPService) updateAppleSubscriptionFromTransaction(ctx context.Context, transactionInfo *AppleTransactionInfo, notification *IAPNotification, updateFunc func(*models.AppleSubscription)) error {
	// 1. 查找订阅记录
	var appleSub models.AppleSubscription
	err := models.DataBase().WithContext(ctx).
		Where("original_transaction_id = ?", transactionInfo.OriginalTransactionID).
	...
	*appleSub.LastNotificationDate = time.Now()
	...
	// 4. 保存到数据库
	if err := models.DataBase().WithContext(ctx).Save(&appleSub).Error; err != nil {
...
func (s *AppleIAPService) createAppleSubscriptionFromTransaction(ctx context.Context, transactionInfo *AppleTransactionInfo, notification *IAPNotification) error {
	...
	// 创建新的订阅记录
	appleSub := models.AppleSubscription{
		UserID:                uint64(userID),
	...
	*appleSub.LastNotificationDate = time.Now()
	...
	if err := models.DataBase().WithContext(ctx).Create(&appleSub).Error; err != nil {
