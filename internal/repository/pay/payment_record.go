...
type PaymentRecord struct {
	...
	PaymentProvider string        `gorm:"column:payment_provider;size:50;not null" json:"payment_provider"`   // 支付提供商（App Store/Google Play）
	TransactionID   string        `gorm:"column:transaction_id;size:255;uniqueIndex" json:"transaction_id"`   // IAP 交易ID
	OriginalTxID    string        `gorm:"column:original_tx_id;size:255;index" json:"original_tx_id"`         // 原始交易ID（用于订阅续费）
	ProductID       string        `gorm:"column:product_id;size:255;index" json:"product_id"`                 // 产品ID
	PurchaseToken   string        `gorm:"column:purchase_token;type:text" json:"purchase_token"`              // 购买令牌（Google IAP）
	ReceiptData     string        `gorm:"column:receipt_data;type:text" json:"receipt_data"`                  // 收据数据（Apple IAP）
	PaymentTime     *time.Time    `gorm:"column:payment_time" json:"payment_time"`                            // 支付时间
	ExpiresTime     *time.Time    `gorm:"column:expires_time" json:"expires_time"`                            // 过期时间（订阅用）
	...
	Environment     string        `gorm:"column:environment;size:20;default:'Production'" json:"environment"` // 环境（Production/Sandbox）
	...
	VerificationHash string `gorm:"column:verification_hash;size:255" json:"verification_hash"`  // 验证哈希
