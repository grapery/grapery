...
func (s *UsageLimitStrategyImpl) GetUserLimitStatus(ctx context.Context, userID int64) (*UserLimitStatus, error) {
	...
	return &UserLimitStatus{
		...
		IsVip:              subscription.Status == paymodels.UserSubscriptionStatusActive,
		VipLevel:           vipLevel,
...
func (s *UsageLimitStrategyImpl) GetFeatureLimits(ctx context.Context, userID int64) (map[string]*FeatureLimit, error) {
	// 获取用户订阅状态
	_, err := paymodels.GetUserActiveSubscriptionByUserID(ctx, userID)
	if err != nil {
		// 没有活跃订阅，返回默认限制
...
func (s *UsageLimitStrategyImpl) getUsageTypeFromAction(action ActionType) paymodels.TokenUsageType {
	switch action {
	case ActionTypeChat:
...
func (s *UsageLimitStrategyImpl) getUsageTypeString(usageType paymodels.TokenUsageType) string {
	switch usageType {
...
func (s *UsageLimitStrategyImpl) adjustPermissionBySubscription(permission *ActionPermission, subscription *paymodels.UserSubscription, action ActionType) *ActionPermission {
	// 如果已经是VIP用户，即使用量超限也保持会员身份
	if subscription.Status == paymodels.UserSubscriptionStatusActive {
		// VIP用户超限时的特殊处理
		if !permission.CanPerform {
...
func (s *UsageLimitStrategyImpl) getVipUpgradeSuggestion(subscription *paymodels.UserSubscription, action ActionType) string {
	// 根据当前订阅等级和操作类型提供升级建议
	vipLevel := s.determineVipLevel(subscription)
...
func (s *UsageLimitStrategyImpl) determineVipLevel(subscription *paymodels.UserSubscription) string {
	// 根据订阅的配额限制确定VIP等级
...
func (s *UsageLimitStrategyImpl) getSubscriptionStatusString(status paymodels.UserSubscriptionStatus) string {
	switch status {
	case paymodels.UserSubscriptionStatusActive:
		return "active"
	case paymodels.UserSubscriptionStatusExpired:
		return "expired"
	case paymodels.UserSubscriptionStatusCanceled:
		return "canceled"
	case paymodels.UserSubscriptionStatusPending:
		return "pending"
	case paymodels.UserSubscriptionStatusFailed:
		return "failed"
	case paymodels.UserSubscriptionStatusTrial:
		return "trial"
	case paymodels.UserSubscriptionStatusPaused:
		return "paused"
	default:
		return "unknown"
	}
...
func (s *UsageLimitStrategyImpl) calculateOverallUsage(ctx context.Context, userID int64) (*UsageLimitResult, error) {
	// 获取所有类型的用量统计
	stats, err := s.tokenUsageService.GetUserUsageStats(ctx, userID, paymodels.TokenUsagePeriodMonthly)
	if err != nil {
		return nil, err
	}
