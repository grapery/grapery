...
func (s *Service) CreateStoryboard(ctx context.Context, storyboard *domain.Storyboard) error {
	...
	if err := s.repo.IncrementStoryStoryboardCount(ctx, storyboard.StoryID); err != nil {
		...
		s.invalidateStoryCache(ctx, storyboard.StoryID)
...
type storyboardFeedCachePayload struct {
	Storyboards []*domain.Storyboard `json:"storyboards"`
...
func (s *Service) getRecommendedStoryboardFeed(ctx context.Context, userID string, limit, offset int) ([]*domain.Storyboard, int64, error) {
	...
	page := (offset / limit) + 1
	cacheTTL := time.Duration(s.recoCfg.CacheTTLSeconds) * time.Second
	if cacheTTL <= 0 {
		cacheTTL = 3 * time.Minute
	}
	cacheKey := recommendation.FeedCacheKey(recommendation.FeedTypeStoryboards, userID, page, limit)
	if c != nil {
		var cached storyboardFeedCachePayload
		if err := c.Get(ctx, cacheKey, &cached); err == nil {
			s.logger.Debug("storyboard for_you feed cache hit",
				zap.String("userID", userID),
				zap.Int("count", len(cached.Storyboards)))
			if s.metrics != nil {
				s.metrics.RecordCacheHit("storyboard_feed")
	...
	if c != nil {
		seen, errSeen := recommendation.ListSeenStoryboardIDs(ctx, c, s.recoCfg, userID)
		if errSeen != nil {
			s.logger.Warn("list seen storyboards for feed", zap.Error(errSeen))
		} else {
			exclude = recommendation.SeenIDsToSet(seen)
		}
	}
	storyboards, total, err := s.repo.StoryboardFeedRecommended(ctx, userID, limit, offset, exclude)
	if err != nil {
		return nil, 0, err
	}
	if c != nil && len(storyboards) > 0 {
...
func (s *Service) excludeBlockedCreators(ctx context.Context, userID string, storyboards []*domain.Storyboard, total int64) ([]*domain.Storyboard, int64) {
	...
	blockedIDs, err := s.ListBlockedUserIDs(ctx, userID)
	if err != nil || len(blockedIDs) == 0 {
		return storyboards, total
	}
	blocked := BlockedIDSet(blockedIDs)
	return ExcludeByBlockedAuthors(storyboards, total, blocked, func(sb *domain.Storyboard) string {
		if sb == nil {
			return ""
		}
		return sb.UserID
...
func (s *Service) GenerateStoryboardWithAI(ctx context.Context, storyboard *domain.Storyboard) error {
	...
	return s.generateStoryboardWithRedesignPipeline(ctx, storyboard, story)
}
...
func (s *Service) buildStoryboardPrompt(storyboard *domain.Storyboard, story *domain.Story, contextInfo string, continuation bool) string {
	...
	if continuation {
		...
		prompt += "5. 每个场景须相对上一场景产生新信息、新行动或新后果；不要只换镜头重复同一局势\n"
		prompt += "6. 若续写对象是动物、器物、建筑或静物，保持其非人类身份，用可见动作/姿态/损耗表现目标与代价\n\n"
	} else {
		...
		prompt += "5. 写作前先确定主体、欲望、阻碍、行动、代价/悬念；输出中不用列出，但每个场景应体现其中一环\n"
		prompt += "6. 主体可以是人物、动物、拟人器物或静物；无人类故事也要有可观察的目标与阻碍，不要强行加入无关人物\n\n"
	}
	...
	prompt += "      \"description\": \"场景描述（100-200字）。同时写清剧情功能（setup/inciting/attempt/reversal/cost/payoff 之一，不必显式标注）与视觉导向语言：摄影机视角、光影、主体动作/表情/姿态、环境质感与氛围。\",\n"
	prompt += "      \"location\": \"地点\",\n"
	prompt += "      \"timeOfDay\": \"时间\",\n"
	...
	prompt += "\n重要：所有 scenes 连起来必须像一段可读小故事，能用“因为/但是/所以”串联；至少三场景时，必须出现一次局势改变和一次后果/余韵。"
	prompt += "\n重要：若主体不是人类，characters 可为空或只包含真实参与的已有角色，但 description 仍要把动物/器物/静物主体写清楚，避免为了填角色而硬加人。"
	...
	prompt += "\n重要：不要把所有分镜写成风景插画。若故事出现主要转折、震惊、期待、庆祝、心理犹豫或明确台词，必须在对应场景中通过 comicTexts、特写、效果线、留白、旁白框、气泡安全区等漫画语言突出；可自然使用短语气词如「啊？」「……」「要来了」「终于」「太好了！」。"
...
2. **剧情合理拓展**：在画面感之上补足因果——每场景相对上一场景至少有一项局势、认知或情绪变化；整板须能串成说得通的微型故事弧，禁止多场景仅换机位、情节静止
3. **叙事主体多样**：主角可以是人物、动物、拟人器物或静物；无人类时仍须写出可观察的目标/恐惧/执念，拟人表演须可画（姿态、裂纹、倾斜、拟声等）
4. **自由发挥**：根据故事主旨和情节需要，智能选择最合适的角色和场景，不必拘泥于使用所有可用资源
5. **连贯性**：确保场景之间有自然的过渡和逻辑联系；新增元素须从用户描述或前场景合理外推
6. **画面感**：每个场景描述应具有强烈的视觉冲击力，便于后续图像/视频生成
7. **角色一致性**：使用角色时严格遵循其设定，保持行为和气质的连贯
8. **情感递进**：场景间应有情感的起伏变化，避免单调
...
- 场景描述：100-200字。必须同时承担叙事增量与画面定格：写清本场景相对上一场景的局势变化（触发/反应/后果之一），以及摄影机位、光线、主体表演（人/动物/拟人器物的姿态表情）、环境质感。
...
- **漫画重点场景**：主要转折用大格/断裂边框/突变光色/旁白框突出“局面变了”；震惊用极近景、放射线、汗滴、短语气词（如「啊？」）；期待用留白、画外视线、手部停顿、短旁白或思想泡（如「要来了」「……」）；庆祝用暖光、开阔构图、星形高光/人群反应、短对白（如「太好了！」）；心理描写用 thought 气泡、低饱和背景和眼神/手部细节。
- 地点和时间：具体明确，与场景内容呼应
