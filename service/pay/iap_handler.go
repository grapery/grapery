...
import (
	...
	"github.com/grapery/grapery/service/pay/middleware"
	"github.com/sirupsen/logrus"

	mainmodels "github.com/grapery/grapery/models"
)
...
func (h *IAPHandler) AcknowledgePurchase(c *gin.Context) {
	...
	// Create system notification for VIP subscription
	go func() {
		bgCtx := context.Background()
		userID64 := int64(userID)
		notifErr := llmchatservice.CreateSystemNotification(bgCtx, llmchatservice.CreateNotificationParams{
			UserID:  userID64,
			Type:    mainmodels.SystemNotificationTypeVIPSubscription,
			Title:   "会员订阅成功",
			Content: fmt.Sprintf("您的%s平台会员订阅已成功激活", req.Platform),
		...
		if notifErr != nil {
			...
			}).Error("Failed to create VIP subscription notification")
		}
	}()
...
func (h *IAPHandler) GetProducts(c *gin.Context) {
	...
	// 确定平台
	var platform models.IAPProductPlatform
	...
	if req.Featured != nil && *req.Featured {
		...
		var productType models.IAPProductType
		switch req.Type {
		case "subscription":
			productType = models.IAPProductTypeSubscription
		case "onetime":
			productType = models.IAPProductTypeOneTime
		case "consumable":
			productType = models.IAPProductTypeConsumable
		}
		h.logger.WithFields(logrus.Fields{
			"platform":     req.Platform,
...
func (h *IAPHandler) GetProductDetail(c *gin.Context) {
	...
	// 确定平台字符串
	platformStr := "apple"
	if product.Platform == models.IAPProductPlatformGoogle {
		platformStr = "google"
		h.logger.WithFields(logrus.Fields{
			"product_id": productID,
