...
type McpService struct {
	...
	storyEngine   story.EngineServer
}

// NewMcpService creates a new MCP service instance
func NewMcpService(engine story.EngineServer) *McpService {
	service := &McpService{
	...
	service.registerDefaultPromptsAndTools()
	return service
}
...
func (s *McpService) HandleRequest(ctx context.Context, req map[string]interface{}) ([]byte, error) {
	...
	ctx = s.injectContextValues(ctx, req)
	...
	case "create_storyboard":
		response, err = s.handleCreateStoryboard(ctx, req)
	case "follow_user":
...
// BuildMCPServer constructs an official Model Context Protocol server with
// Grapery tools, prompts, and resources registered.
func (s *McpService) BuildMCPServer() *mcp.Server {
...
func (s *McpService) injectContextValues(ctx context.Context, req map[string]interface{}) context.Context {
	if trace, ok := req["trace_id"].(string); ok && trace != "" {
		ctx = context.WithValue(ctx, "traceId", trace)
	}
	if userID, ok := getOptionalInt64(req, "user_id"); ok && userID > 0 {
		ctx = context.WithValue(ctx, utils.UserIdKey, userID)
		return ctx
	}
	if creatorID, ok := getOptionalInt64(req, "creator_id"); ok && creatorID > 0 {
		ctx = context.WithValue(ctx, utils.UserIdKey, creatorID)
	}
	return ctx
}
...
func (s *McpService) registerDefaultPromptsAndTools() {
	timestamp := time.Now().Unix()
	for id, prompt := range defaultPromptDefinitions(timestamp) {
		if _, exists := s.prompts[id]; !exists {
			// Create a shallow copy to avoid shared references when timestamp reused
			clone := *prompt
			s.prompts[id] = &clone
		}
	}
	for id, tool := range defaultToolDefinitions(timestamp) {
