...
func (s *McpService) handleCreateStory(ctx context.Context, req map[string]interface{}) ([]byte, error) {
	title, err := getStringField(req, "title")
	if err != nil {
		return nil, err
	}

	shortDesc := getStringOrDefault(req, []string{"summary", "short_desc"}, "")
	description := getStringOrDefault(req, []string{"description", "content", "story"}, "")
	if shortDesc == "" {
		shortDesc = description
	}

	groupID, err := getInt64Field(req, "group_id")
	if err != nil {
		return nil, err
	}

	creatorID, err := getInt64FieldWithFallback(req, "creator_id", "user_id")
	if err != nil {
		return nil, err
	}

	origin := getStringOrDefault(req, []string{"origin", "background"}, "")
	background := getStringOrDefault(req, []string{"background"}, "")
	style := getStringOrDefault(req, []string{"style"}, "")
	subject := getStringOrDefault(req, []string{"subject", "theme"}, "")
	negativePrompt := getStringOrDefault(req, []string{"negative_prompt"}, "")
	prompt := getStringOrDefault(req, []string{"prompt"}, "")
	refImage := getStringOrDefault(req, []string{"ref_image", "reference_image"}, "")
	layoutStyle := getStringOrDefault(req, []string{"layout_style"}, "")

	sceneCount, _ := getOptionalInt64(req, "scene_count")
	isAiGen := true
	if v, ok := getOptionalBool(req, "is_ai_gen"); ok {
		isAiGen = v
	}

	params := &api.StoryParams{}
	if description != "" {
		params.StoryDescription = description
	}
	if background != "" {
	...
	if refImage != "" {
		params.RefImage = refImage
	}
	if layoutStyle != "" {
		params.LayoutStyle = layoutStyle
	}
	if sceneCount > 0 {
		params.SceneCount = int32(sceneCount)
	}
	if params.Background == "" {
		params.Background = origin
	}
	if params.StoryDescription == "" {
		params.StoryDescription = shortDesc
	}

	storyReq := &api.CreateStoryRequest{
		Name:      title,
		Title:     title,
		ShortDesc: shortDesc,
		CreatorId: creatorID,
	...
	resp, err := s.storyEngine.CreateStory(ctx, storyReq)
	if err != nil {
		return nil, fmt.Errorf("create story failed: %w", err)
	}
	if resp == nil || resp.GetCode() != 0 || resp.GetData() == nil {
		message := "unknown error"
		if resp != nil {
			message = resp.GetMessage()
		}
		return nil, fmt.Errorf("create story failed: %s", message)
	}

	storyID := int64(resp.GetData().GetStoryId())

	storyInfoResp, err := s.storyEngine.GetStory(ctx, &api.GetStoryInfoRequest{StoryId: storyID})
	if err != nil {
		log.WithError(err).Warn("failed to fetch story detail after creation")
	}

	var mcpStory *Story
	if storyInfoResp != nil && storyInfoResp.GetData() != nil {
		mcpStory = convertAPIStoryToMCP(storyInfoResp.GetData().GetInfo())
	}
	if mcpStory == nil {
		mcpStory = &Story{
			ID:          fmt.Sprintf("%d", storyID),
			Title:       title,
			...
			AuthorID:    fmt.Sprintf("%d", creatorID),
		}
	}

	s.mu.Lock()
	s.stories[mcpStory.ID] = mcpStory
	s.mu.Unlock()
...
func (s *McpService) handleCreateCharacter(ctx context.Context, req map[string]interface{}) ([]byte, error) {
	name, err := getStringField(req, "name")
	if err != nil {
		return nil, err
	}
	storyID, err := getInt64Field(req, "story_id")
	if err != nil {
		return nil, err
	}
	creatorID, err := getInt64FieldWithFallback(req, "creator_id", "user_id")
	...
	description, err := getStringField(req, "description")
	if err != nil {
		return nil, err
	}

	avatar := getStringOrDefault(req, []string{"avatar", "character_avatar"}, "")
	characterType := getStringOrDefault(req, []string{"character_type"}, "")
	prompt := getStringOrDefault(req, []string{"prompt", "character_prompt"}, "")
	characterID := getStringOrDefault(req, []string{"character_id"}, "")

	refImages, err := toStringSlice(req["ref_images"])
	if err != nil && !isNil(req["ref_images"]) {
		return nil, err
	}

	detailMap, _ := getOptionalMap(req, "detail")
	characterDetail := mapToCharacterDetail(detailMap)
	...
	if characterDetail != nil {
		role.CharacterDetail = characterDetail
	}

	resp, err := s.storyEngine.CreateStoryRole(ctx, &api.CreateStoryRoleRequest{
		UserId: creatorID,
		Role:   role,
	})
	if err != nil {
	...
	if resp == nil || resp.GetCode() != api.ResponseCode_OK {
		message := "unknown error"
		if resp != nil {
			message = resp.GetMessage()
		}
		return nil, fmt.Errorf("create role failed: %s", message)
	}

	roleModel, err := models.GetStoryRoleByName(ctx, name, storyID)
	if err != nil {
		return nil, fmt.Errorf("created role but failed to fetch detail: %w", err)
	}

	convertedRole := modelRoleToCharacter(roleModel)

	s.mu.Lock()
	s.characters[convertedRole.ID] = convertedRole
	if storySummary, exists := s.stories[fmt.Sprintf("%d", storyID)]; exists {
		storySummary.Characters = append(storySummary.Characters, convertedRole.ID)
		storySummary.UpdatedAt = time.Now().Unix()
	}
	s.mu.Unlock()
...
func (s *McpService) handleCreateStoryboard(ctx context.Context, req map[string]interface{}) ([]byte, error) {
	storyID, err := getInt64Field(req, "story_id")
	if err != nil {
		return nil, err
	}
	...
	content := getStringOrDefault(req, []string{"content", "summary", "description"}, "")
	background := getStringOrDefault(req, []string{"background"}, "")
	style := getStringOrDefault(req, []string{"style"}, "")
	negativePrompt := getStringOrDefault(req, []string{"negative_prompt"}, "")
	refImage := getStringOrDefault(req, []string{"ref_image"}, "")
	layoutStyle := getStringOrDefault(req, []string{"layout_style"}, "")
	paramsMap, _ := getOptionalMap(req, "params")

	prevBoardID, _ := getOptionalInt64(req, "prev_board_id")

	boardRoles, err := s.resolveStoryboardRoles(ctx, storyID, creatorID, req)
	if err != nil {
		return nil, err
	}
	if len(boardRoles) == 0 {
		return nil, fmt.Errorf("at least one role is required to create a storyboard")
	}

	boardParams := mapToStoryBoardParams(paramsMap)
	if boardParams == nil {
		boardParams = &api.StoryBoardParams{}
	}
	if style != "" {
		boardParams.Style = style
	}
	if negativePrompt != "" {
		boardParams.NegativePrompt = negativePrompt
	}
	if refImage != "" {
		boardParams.RefImage = refImage
	}
	if layoutStyle != "" {
		boardParams.LayoutStyle = layoutStyle
	}
	if scenes, ok := getOptionalInt64(req, "scene_count"); ok {
		boardParams.SceneCount = int32(scenes)
	}

	board := &api.StoryBoard{
		StoryId:   storyID,
		Creator:   creatorID,
		Title:     title,
		Content:   content,
	...
	if prevBoardID > 0 {
		board.PrevBoardId = prevBoardID
	}

	resp, err := s.storyEngine.CreateStoryboard(ctx, &api.CreateStoryboardRequest{Board: board})
	if err != nil {
		return nil, fmt.Errorf("create storyboard failed: %w", err)
	}
	if resp == nil || resp.GetCode() != 0 || resp.GetData() == nil {
		message := "unknown error"
		if resp != nil {
		...
		return nil, fmt.Errorf("create storyboard failed: %s", message)
	}

	boardID := resp.GetData().GetBoardId()
