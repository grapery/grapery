...
func (ts *UserService) UserGroup(ctx context.Context, req *connect.Request[api.UserGroupRequest]) (*connect.Response[api.UserGroupResponse], error) {
	...
	if err != nil {
		log.Println("UserGroup error: ", err.Error())
		return nil, err
	}
	log.Println("UserGroup info: ", info.String())
...
func (ts *UserService) GetFollowList(ctx context.Context, req *connect.Request[api.GetFollowListRequest]) (*connect.Response[api.GetFollowListResponse], error) {
	uidStr := req.Header().Get(utils.UserIdKey)
	if len(uidStr) == 0 && req.Msg.GetUserId() == 0 {
		return nil, connect.NewError(connect.CodeUnauthenticated, errors.New("unauthenticated"))
	}
	if req.Msg.GetUserId() == 0 {
...
func (ts *UserService) FetchUserGenTaskStatus(ctx context.Context, req *connect.Request[api.FetchUserGenTaskStatusRequest]) (*connect.Response[api.FetchUserGenTaskStatusResponse], error) {
	info, err := userService.GetUserServer().FetchUserGenTaskStatus(ctx, req.Msg)
	if err != nil {
		return nil, err
	}
	return &connect.Response[api.FetchUserGenTaskStatusResponse]{
		Msg: info,
	}, nil
}
